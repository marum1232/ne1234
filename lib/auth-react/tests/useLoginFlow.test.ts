...
describe("useLoginFlow", () => {
  ...
  describe("initiateLogin", () => {
    it("calls check-identifier endpoint and returns method/exists", async () => {
      ...
      await act(async () => {
        checkResult = await result.current.initiateLogin("03001234567");
      });
  ...
  describe("verifyOtp", () => {
    it("calls verify-otp and logs in user on success", async () => {
      ...
      expect(mockFetch.mock.calls[1]?.[0]).toContain("/api/auth/verify-otp");
    ...
    it("calls /api/auth/login and succeeds", async () => {
      mockFetch
        .mockResolvedValueOnce({
          ok: true,
          status: 200,
          json: () => Promise.resolve({ success: true, data: { method: "password", exists: true } }),
        })
        .mockResolvedValueOnce({
          ok: true,
          status: 200,
  ...
  describe("twoFactorVerify", () => {
    it("calls /api/auth/2fa/verify and calls onSuccess", async () => {
      ...
      expect(mockFetch.mock.calls[2]?.[0]).toContain("/api/auth/2fa/verify");
      expect(onSuccess).toHaveBeenCalledWith(
