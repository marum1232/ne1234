...
describe('useAuth — return shape', () => {
  ...
  it('login() sets user and isAuthenticated', () => {
    ...
    const fakeToken = 'header.' + btoa(JSON.stringify({ exp: Math.floor(Date.now() / 1000) + 3600 })) + '.sig';
  ...
  it('logout() clears user', () => {
    ...
    act(() => { result.current.login(fakeUser, fakeToken); });
    expect(result.current.isAuthenticated).toBe(true);
...
describe('useTokenRefresh — return shape and options', () => {
  it('returns { refreshToken } function', () => {
    ...
    const { result } = renderHook(() =>
      useTokenRefresh({ tokenStorage: storage, baseURL: '' })
    );
    expect(typeof result.current.refreshToken).toBe('function');
  });
...
describe('useSessionManager — return shape', () => {
  it('returns expected keys', () => {
    mockFetch.mockResolvedValue({ ok: true, json: () => Promise.resolve({ success: true, data: { sessions: [] } }) });
