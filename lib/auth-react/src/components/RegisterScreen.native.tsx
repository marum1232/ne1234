            ...
            currentStep.fields.filter((f) => f.type !== 'otp').map((field) => (
              <View key={field.id} style={styles.fieldWrap}>
                ...
                {field.type === 'phone' ? (
                  ...
                  <View style={styles.selectContainer}>
                    {field.options?.map((opt) => (
                      <TouchableOpacity
                        ...
                        onPress={() => { updateField(field.id, opt.value); }}
                        accessibilityRole="radio"
                        accessibilityState={{ selected: (formData[field.id] as string) === opt.value }}
                      >
                        <Text
                          style={[
                            styles.selectOptionText,
                            (formData[field.id] as string) === opt.value && { color: accent, fontWeight: '700' },
                          ]}
                        >
                          {opt.label}
                        </Text>
                  ...
                  >
                    <View style={[styles.checkbox, !!formData[field.id] && { borderColor: accent, backgroundColor: accent }]}>
                  ...
                  <TextInput
                    ...
                    secureTextEntry={field.type === 'password' || field.type === 'confirm-password' || field.secureTextEntry}
                    keyboardType={field.keyboardType ?? (field.type === 'email' ? 'email-address' : 'default')}
...
const styles = StyleSheet.create({
  ...
  card: { backgroundColor: '#fff', borderRadius: 16, padding: 24, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.08, shadowRadius: 12, elevation: 4 },
  errorBox: { backgroundColor: '#fef2f2', borderRadius: 8, padding: 12, marginBottom: 16, borderWidth: 1, borderColor: '#fca5a5' },
  errorText: { color: '#b91c1c', fontSize: 13 },
  devOtpBox: { backgroundColor: '#fef3c7', borderRadius: 8, padding: 10, marginBottom: 12, borderWidth: 1, borderColor: '#fcd34d' },
  devOtpLabel: { fontSize: 10, fontWeight: '700', color: '#92400e', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 2 },
  ...
  input: { height: 48, borderWidth: 1.5, borderColor: '#d1d5db', borderRadius: 10, paddingHorizontal: 14, fontSize: 15, color: '#111827', backgroundColor: '#f9fafb' },
  selectContainer: { flexDirection: 'row', flexWrap: 'wrap', gap: 8 },
  selectOption: { paddingHorizontal: 14, paddingVertical: 10, borderRadius: 8, borderWidth: 1.5, borderColor: '#e5e7eb' },
