...
router.patch("/:id/location", riderAuth, gpsAntiSpoofMiddleware, async (req, res) => {
  ...
  const [booking] = await db
    .select({ id: parcelBookingsTable.id, riderId: parcelBookingsTable.riderId, status: parcelBookingsTable.status })
    .from(parcelBookingsTable)
    .where(eq(parcelBookingsTable.id, bookingId))
    .limit(1);

  if (!booking) { sendNotFound(res, "Parcel booking not found"); return; }
  if (booking.riderId !== riderId) { sendForbidden(res, "You are not the assigned rider for this parcel"); return; }

  const activeStatuses = ["accepted", "picked_up", "in_transit"];
  if (!activeStatuses.includes(booking.status ?? "")) {
    sendError(res, "Live location updates are only accepted for active parcel deliveries", 409); return;
  }
