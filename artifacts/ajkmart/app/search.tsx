...
function UniversalSearchScreenInner() {
  ...
  const [sections, setSections] = useState<
    { title: string; data: SearchResult[]; type: ServiceKey }[]
  >([]);
  ...
  const fetchResults = useCallback(
    async (q: string, fromExplicit = false, page = 1) => {
      const effectiveCategory = filterCategory || activeCategory;
      if (!q.trim() && !effectiveCategory) {
        setSections([]);
        setTotalPages(0);
        setCurrentPage(1);
        return;
      ...
      try {
            ...
            if (page > 1) {
              ...
              if (existingIdx >= 0) {
                const existingIds = new Set(
                  (sections[existingIdx]?.data ?? []).map((d) => d.id),
                );
                const newItems = result.value.products.filter(
        ...
        if (page > 1) {
          const unchanged = sections.filter(
            (sec) => !newSections.some((ns) => ns.type === sec.type),
          );
      ...
      <View style={s.header}>
        ...
        <View style={s.inputWrap}>
          ...
          <TextInput
            ...
            maxLength={200}
            onSubmitEditing={() => {
              if (query.trim()) fetchResults(query, true);
            }}
          />
          {query.length > 0 && (
            <TouchableOpacity
      ...
      <View style={s.filterBar}>
        ...
        >
          ...
          <TouchableOpacity
            ...
            style={[s.sortChip, showFilters && s.sortChipActive]}
          >
            <Ionicons
              name="options-outline"
              size={13}
              color={showFilters ? "#fff" : C.textSecondary}
            />
            <Text style={[s.sortChipTxt, showFilters && s.sortChipTxtActive]}>
              Filters
            </Text>
          </TouchableOpacity>
        </ScrollView>
            ...
            <View style={s.filterSection}>
              ...
              >
                <TouchableOpacity
                  ...
                  style={[
                    s.ratingChip,
                    filterCategory === "" && s.ratingChipActive,
                  ]}
                >
                  <Text
                    style={[
                      s.ratingChipTxt,
                      filterCategory === "" && s.ratingChipTxtActive,
                    ]}
                  >
                    All
                  </Text>
                ...
                {allCategories.map((cat) => (
                  <TouchableOpacity
                    ...
                    style={[
                      s.ratingChip,
                      filterCategory === cat.id && s.ratingChipActive,
                    ]}
                  >
                    <Text
                      style={[
                        s.ratingChipTxt,
                        filterCategory === cat.id && s.ratingChipTxtActive,
                      ]}
                    >
                      {cat.name}
                    </Text>
              ...
              <View style={s.ratingChipsRow}>
                {SORT_OPTIONS.map((opt) => (
                  <TouchableOpacity
                    ...
                    style={[
                      s.ratingChip,
                      sortBy === opt.key && s.ratingChipActive,
                    ]}
                  >
                    <Text
                      style={[
                        s.ratingChipTxt,
                        sortBy === opt.key && s.ratingChipTxtActive,
                      ]}
                    >
                      {T(opt.labelKey)}
                    </Text>
                  ...
                  return (
                    ...
                    >
                      ...
                      <Text
                        style={[
                          s.ratingChipTxt,
                          isSelected && s.ratingChipTxtActive,
                        ]}
                      >
                        {label}
                      </Text>
      ...
      {!loading &&
        ...
        searchError && (
          <View style={s.center}>
            ...
            <Text style={[s.emptyTxt, { color: "#EF4444" }]}>
              Search failed
            </Text>
        ...
        !searchError && (
          <View style={s.center}>
            ...
            <Text style={s.emptySub}>
              Try a different keyword or browse a service
            </Text>
            <View style={s.noResultsCtaRow}>
              {enabledServices
                .filter((sv) => sv !== "pharmacy")
                .map((sv) => {
                  ...
                  return (
                    <TouchableOpacity
                      activeOpacity={0.7}
                      key={sv}
                      onPress={() => router.push(SERVICE_ROUTES[sv])}
                      style={[s.ctaBtn, { backgroundColor: m.bg }]}
                    >
                      <Ionicons name={m.icon} size={14} color={m.color} />
                      <Text style={[s.ctaBtnTxt, { color: m.color }]}>
                        Browse {m.label}
                      </Text>
                    </TouchableOpacity>
                  );
              ...
              {enabledServices.map((sv) => {
                ...
                return (
                  <TouchableOpacity
                    ...
                    key={sv}
                    onPress={() => router.push(SERVICE_ROUTES[sv])}
                    style={[s.browseCard, { backgroundColor: m.bg }]}
                  >
                    <View
                      style={[
                        s.browseIconWrap,
                    ...
                    <Text style={[s.browseLabel, { color: m.color }]}>
                      {m.label}
                    </Text>
                  </TouchableOpacity>
                );
        ...
        <SectionList
          ...
          renderSectionHeader={({ section }) => (
            <View style={s.sectionHeader}>
              <Ionicons
                name={SERVICE_META[section.type].icon}
                size={14}
                color={SERVICE_META[section.type].color}
              />
              <Text
                style={[
                  s.sectionTitle,
              ...
              <View
                style={[
                  s.sectionCount,
                  { backgroundColor: SERVICE_META[section.type].bg },
                ]}
              >
                <Text
                  style={[
              ...
              <View
                style={[
                  s.sectionDivider,
                  { backgroundColor: SERVICE_META[section.type].color + "30" },
                ]}
              />
            </View>
          )}
            ...
            >
              <View style={s.cardImgWrap}>
                {item.image ? (
                  ...
                  <Ionicons
                    name={SERVICE_META[item.type].icon}
                    size={20}
                    color={C.textMuted}
                  />
                )}
                ...
                item.type !== "pharmacy" ? (
                  ...
                  >
                    <Ionicons
                      ...
                      color={C.primary}
                    />
                    <Text style={s.cardVendorLink} numberOfLines={1}>
                      {item.vendorName}
                    </Text>
                  </TouchableOpacity>
                ) : item.vendorName ? (
