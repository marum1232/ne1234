...
export function usePwaInstall() {
  ...
  const promptInstall = async () => {
    ...
    } catch (err) { console.warn('[artifacts/rider-app/src/hooks/usePwaInstall.ts]', err); } // eslint-disable-line no-console
  ...
  const dismiss = () => {
    setIsDismissed(true);
    try { localStorage.setItem(DISMISSED_KEY, "1"); } catch (err) { console.warn('[artifacts/rider-app/src/hooks/usePwaInstall.ts]', err); } // eslint-disable-line no-console
  };
